...
\vskip 3ex
$\Huge\frac{P_{\theta}(\tau)}{\overline{P}(\tau)} =
 \frac{\cancel{P(s_1)} \Pi_{t=1}^T\pi_{\theta}(a_t,s_t)     \cancel{P(s_{t+1} \mid (s_t,a_t))}}{\cancel{P(s_1)} \Pi_{t=1}^T\overline{\pi}_{\theta}(a_t,s_t) \cancel{P(s_{t+1} \mid (s_t,a_t))}}$
