...
\scalebox{1.5}{%
$\nabla_{\theta} J(\theta) \sim \frac{1}{N} \sum_{i=1}^N \sum_{t=1}^T \nabla_{\theta} \log \pi{\theta} (a_t^i s_t^i) \underbrace{\tcboxmath{r(s_{i,{t^{\prime}}}, a_{i,{t^{\prime}}})}}_{\parbox{3cm}{Extimate future rewards if we take {a_t} \text{ at } {s_t}}}$}

True future rewards can be obtained by using the $\mathbb{Q}$ function like this.


\scalebox{1.8}{%
  $\sum_{t^{\primt} = t}^T\mathbb{E}\left r (s_{t^{\prime}}, a_{t^{\prime}}) \mid r(s_t, a_t)[\right] = \mathbb{Q}(s_t, a_t)$
 }
\end{document}
